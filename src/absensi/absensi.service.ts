...
export class AbsensiService {
  ...
  async findAll(query: Query): Promise<any> {
    ...
    if (!query.page && !query.keyword) {
      ...
      absensi = await this.absensiModel
        .find()
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(resPerPage)
      ...
      totalData = await this.absensiModel
  ...
  async createAbsentDataForCurrentDay(): Promise<void> {
    ...
    for (const user of users) {
      ...
      const absensi = await this.absensiModel
        .findOne({ 'user._id': user._id, date: currentDate })
        .exec();
      const cuti = await this.cutiModel
        .findOne({ 'user._id': user._id, date: currentDate })
        .exec();
      const izin = await this.izinModel
        .findOne({ 'user._id': user._id, date: currentDate })
        .exec();
      ...
      if (absensi || cuti || izin) {
        console.log(
          `Pengguna ${user.name} sudah memiliki data pada tanggal ${currentDate}`,
        );
      ...
      const isHoliday = nationalHolidays.includes(
      ...
      await data.save();
      console.log(
        `Data "${type}" ditambahkan untuk pengguna ${user.name} pada tanggal ${currentDate}`,
      );
    }
  }

  async getAllDataSortedByDate(
    query: Query,
    ...
    const allData = [
      ...data1,
      ...data2,
      ...data3 /* Add data2 and data3 here... */,
    ];
  ...
  async createAbsensi(absensi: any, user: User, image: BufferedFile) {
    const userId = user._id;
    const absenTime = new Date(); // Membuat objek tanggal dan waktu saat ini

    const today = new Date(); // Membuat objek tanggal dan waktu saat ini
    today.setHours(0, 0, 0, 0);
    const checkAbsen = await this.absensiModel.findOne({
      date: {
  ...
  async updateCheckout(
    id: string,
    updateAbsensiDto: UpdateAbsensiDto,
  ): Promise<Absensi> {
