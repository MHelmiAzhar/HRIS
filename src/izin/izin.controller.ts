import { Body, Controller, Post, Query, Req, UseGuards, Get, Param, Patch, UploadedFile, UseInterceptors, HttpException, HttpStatus } from '@nestjs/common';
...
export class IzinController {
    ...
    @Patch('approved/:id')
    @UseGuards(AuthGuard())
    async updateApproved(@Param('id') id: string, @Body() updateIzinDto: UpdateIzinDto, @Req() req,): Promise<Izin> {
